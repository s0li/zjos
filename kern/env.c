...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	void* low = ROUNDDOWN(va, PGSIZE);
	void* high = ROUNDUP(va + len, PGSIZE);
	struct Page* p;

	while (low < high) {
		p = page_alloc(0);
		if (p == NULL)
			panic("(region_alloc) page allocation failed, va = %x", low);

		if (page_insert(e->env_pgdir, p, low, PTE_U | PTE_W) < 0)
